...
class UnetPP3D(nn.Module):
    def __init__(self):
        ...
        #Level-4
        self.pool30=nn.MaxPool3d(kernel_size=5,stride=2,padding=2)
        
        self.conv40=nn.Sequential(
            nn.Conv3d(15, 25, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(25),
            nn.ReLU(),
            nn.Conv3d(25, 25, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(25),
            nn.ReLU(),   
        )
        
        self.up40=nn.Sequential(
            nn.ConvTranspose3d(25, 25,kernel_size=6,stride=2,padding=2),
            nn.BatchNorm3d(25),
            nn.ReLU(),
        )
        #conv30+conv40 15 25
        self.conv31=nn.Sequential(
            nn.Conv3d(40, 15, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(15),
            nn.ReLU(),
            nn.Conv3d(15, 15, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(15),
        ...
        #conv31+conv21+conv20 15 10 12
        self.conv22=nn.Sequential(
            nn.Conv3d(37, 15, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(15),
            nn.ReLU(),
            nn.Conv3d(15, 15, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(15),
        ...
        #conv22+conv12+conv11+conv10 15 15 12 8
        self.conv13=nn.Sequential(
            nn.Conv3d(50, 25, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(25),
            nn.ReLU(),
            nn.Conv3d(25, 25, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(25),
        ...
        #conv13+conv03+conv02+conv01+conv00 25 10 11 6 4
        self.conv04=nn.Sequential(
            nn.Conv3d(56, 25, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(25),
            nn.ReLU(),
            nn.Conv3d(25, 25, kernel_size=5, stride=1, padding=2, groups=1, bias=True),
            nn.BatchNorm3d(25),
        ...
        self.final=nn.Conv3d(25, 1, kernel_size=1)
    ...
    def forward(self,spad):
        ...
        #level-4
        x40=self.conv40(self.pool30(x30))
        x31=self.conv31(torch.cat([x30,self.up40(x40)],1))
        x22=self.conv22(torch.cat([x20,x21,self.up31(x31)],1))
        x13=self.conv13(torch.cat([x10,x11,x12,self.up22(x22)],1))
        x04=self.conv04(torch.cat([x00,x01,x02,x03,self.up13(x13)],1))
